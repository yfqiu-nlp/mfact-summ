...
from dexpert_logit_processor import DexpertLogitsProcessor
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # for Dexeprt decoding
            self.set_active_adapters(ac.Stack("wikitext", "summarization"))
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            self.set_active_adapters(ac.Stack("wikitext", "expert_summarization"))
            expert_outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            self.set_active_adapters(ac.Stack("wikitext", "anti_expert_summarization"))
            anti_expert_outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
            ...
            # Dexpert Merging
            dexpert_alpha=0.5
            dexpert_alpha = torch.tensor(dexpert_alpha).to(self.device)
            ensemble_logits = outputs.logits + dexpert_alpha * (expert_outputs.logits - anti_expert_outputs.logits)
            outputs.logits = ensemble_logits
            ## End for Dexpert decoding
